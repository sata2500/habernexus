...
class AuthorCategoryMapping(models.Model):
    ...
    author = models.ForeignKey("authors.Author", on_delete=models.CASCADE, related_name="category_mappings")
    category = models.CharField(max_length=100, help_text="Kategori adı (Teknoloji, Sağlık, vb.)")
    expertise_level = models.IntegerField(
        choices=EXPERTISE_LEVELS, default=1, help_text="Yazarın bu kategorideki uzmanlık seviyesi"
    )
    is_primary = models.BooleanField(default=False, help_text="Bu yazarın birincil kategorisi mi?")
...
class PromptTemplate(models.Model):
    ...
    name = models.CharField(max_length=255, unique=True, help_text="Template adı (örn: 'Teknoloji Makalesi')")
    ...
    is_active = models.BooleanField(default=True, help_text="Bu template aktif mi?")

    version = models.IntegerField(default=1, help_text="Template versiyonu")
...
class ArticleSEO(models.Model):
    ...
    article = models.OneToOneField("Article", on_delete=models.CASCADE, related_name="seo")

    # Meta Tag'ları
    meta_description = models.CharField(max_length=160, help_text="Google'da görünecek açıklama (max 160 karakter)")

    meta_keywords = models.CharField(max_length=255, help_text="Anahtar kelimeler (virgülle ayrılmış)")

    # Canonical URL
    canonical_url = models.URLField(blank=True, help_text="Canonical URL (duplicate content'i önlemek için)")

    # Open Graph Tag'ları (Sosyal Medya)
    og_title = models.CharField(max_length=95, help_text="Sosyal medyada görünecek başlık")

    og_description = models.CharField(max_length=200, help_text="Sosyal medyada görünecek açıklama")

    og_image = models.URLField(blank=True, help_text="Sosyal medyada görünecek görsel URL")

    # Twitter Card'ları
    twitter_title = models.CharField(max_length=70, blank=True, help_text="Twitter'da görünecek başlık")

    twitter_description = models.CharField(max_length=200, blank=True, help_text="Twitter'da görünecek açıklama")
    ...
    seo_score = models.IntegerField(
        default=0, validators=[MinValueValidator(0), MaxValueValidator(100)], help_text="Genel SEO puanı (0-100)"
    )

    readability_score = models.IntegerField(
        default=0, validators=[MinValueValidator(0), MaxValueValidator(100)], help_text="Okunabilirlik puanı (0-100)"
    )
...
class ArticleMedia(models.Model):
    ...
    article = models.OneToOneField("Article", on_delete=models.CASCADE, related_name="media")

    # ===== GÖRSEL =====
    # Orijinal görsel
    featured_image_original = models.ImageField(
        upload_to="articles/original/", null=True, blank=True, help_text="Orijinal görsel dosyası"
    )

    # Optimize edilmiş görseller
    featured_image_avif = models.ImageField(
        upload_to="articles/featured/", null=True, blank=True, help_text="AVIF format (en iyi sıkıştırma)"
    )

    featured_image_webp = models.ImageField(
        upload_to="articles/featured/", null=True, blank=True, help_text="WebP format (fallback)"
    )

    featured_image_jpeg = models.ImageField(
        upload_to="articles/featured/", null=True, blank=True, help_text="JPEG format (legacy)"
    )
    ...
    summary_video_original = models.FileField(
        upload_to="articles/video/", null=True, blank=True, help_text="Orijinal video dosyası"
    )

    # Encode edilmiş videolar
    summary_video_1080p = models.FileField(
        upload_to="articles/video/", null=True, blank=True, help_text="1080p video (yüksek kalite)"
    )
    ...
    summary_video_hls_manifest = models.FileField(
        upload_to="articles/video/hls/", null=True, blank=True, help_text="HLS manifest dosyası"
    )

    # Video metadata
    video_duration = models.IntegerField(null=True, blank=True, help_text="Video uzunluğu (saniye)")
...
class MediaProcessingLog(models.Model):
    ...
    article = models.ForeignKey("Article", on_delete=models.CASCADE, related_name="media_logs")

    media_type = models.CharField(max_length=50, choices=MEDIA_TYPES, help_text="Medya türü")

    status = models.CharField(max_length=50, choices=STATUSES, help_text="İşleme durumu")

    # Dosya boyutları
    original_size = models.BigIntegerField(help_text="Orijinal dosya boyutu (bytes)")

    optimized_size = models.BigIntegerField(null=True, blank=True, help_text="Optimize edilmiş dosya boyutu (bytes)")

    compression_ratio = models.FloatField(null=True, blank=True, help_text="Sıkıştırma oranı (%)")

    # Performans
    processing_time = models.FloatField(help_text="İşleme süresi (saniye)")

    # Hata
    error_message = models.TextField(blank=True, help_text="Hata mesajı (varsa)")

    # Metadata
    source_url = models.URLField(blank=True, help_text="Medya kaynağı URL")
