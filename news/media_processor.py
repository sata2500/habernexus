...
class ImageProcessor:
    ...
    QUALITY_LEVELS = {
        'high': {
            'avif': {'quality': 90, 'crf': 23},
            'webp': {'quality': 85},
            'jpeg': {'quality': 90}
        },
        'medium': {
            'avif': {'quality': 80, 'crf': 28},
            'webp': {'quality': 75},
            'jpeg': {'quality': 80}
        },
        'low': {
            'avif': {'quality': 70, 'crf': 35},
            'webp': {'quality': 65},
            'jpeg': {'quality': 70}
        }
    }
    ...
    def __init__(self, output_dir: str = '/media'):
        ...
        self.session.headers.update({
            'User-Agent': 'HaberNexus/2.0 (+http://habernexus.com)'
        })
    ...
    def _optimize_image(
        ...
        try:
            ...
            image.save(
                avif_path,
                'AVIF',
                quality=quality_settings['avif']['quality']
            )
        ...
        try:
            ...
            image.save(
                jpeg_path,
                'JPEG',
                quality=quality_settings['jpeg']['quality']
            )
    ...
    def _create_metadata(
        ...
        optimized_size = sum(
            os.path.getsize(path) for path in optimized_paths.values()
...
class VideoProcessor:
    ...
    def download_and_encode(
        ...
        try:
            ...
            # Metadata oluştur
            metadata = self._create_video_metadata(
                video_path,
                encoded_videos,
                hls_manifest
            )
    ...
    def process_article_media(
            ...
            for idx, image_url in enumerate(image_urls[:3]):  # İlk 3 görseli al
                try:
                    image_metadata = self.image_processor.download_and_optimize(
                        image_url,
                        article_id,
                        quality='high' if idx == 0 else 'medium'
                    )
        ...
        if video_urls:
            for video_url in video_urls[:1]:  # İlk videoyu al
                try:
                    video_metadata = self.video_processor.download_and_encode(
                        video_url,
                        article_id
                    )
