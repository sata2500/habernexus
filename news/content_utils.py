...
class ContentQualityScorer:
    ...
    def score_headline(headline: str, category: str = None) -> float:
        ...
        # Ağırlıklı ortalama
        total_score = sum(
            scores[key] * ContentQualityScorer.QUALITY_WEIGHTS[key]
            for key in scores
        )
...
class AuthorStyleSelector:
    ...
    def select_author_and_style(category: str, importance_level: int) -> Tuple:
        ...
        mappings = AuthorCategoryMapping.objects.filter(
            category=category,
            author__is_active=True
        ).order_by('-expertise_level', '-is_primary')
