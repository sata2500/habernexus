...
from news.models_advanced import (
    MediaProcessingLog,
    ArticleSEO
)
...
class QualityMetrics:
    ...
    def calculate_media_processing_stats() -> Dict:
        ...
        """
        logs = MediaProcessingLog.objects.filter(
            created_at__gte=timezone.now() - timedelta(hours=24)
        )
        ...
        return {
            'images': {
                ...
                'avg_compression': image_logs.filter(
                    status='completed'
                ).aggregate(Avg('compression_ratio'))['compression_ratio__avg'] or 0,
                'avg_processing_time': image_logs.filter(
                    status='completed'
            ...
            'videos': {
                ...
                'avg_compression': video_logs.filter(
                    status='completed'
                ).aggregate(Avg('compression_ratio'))['compression_ratio__avg'] or 0,
                'avg_processing_time': video_logs.filter(
                    status='completed'
                ).aggregate(Avg('processing_time'))['processing_time__avg'] or 0
            }
        }
    ...
    def get_overview_metrics() -> Dict:
        ...
        return {
            'today': {
                ...
                'avg_quality_score': Article.objects.filter(
                    created_at__gte=today
...
class PerformanceAnalyzer:
    ...
    def get_bottlenecks(days: int = 7) -> List[Dict]:
        ...
        """
        steps = ContentGenerationLog.objects.filter(
            created_at__gte=timezone.now() - timedelta(days=days)
        ).values_list('step', flat=True).distinct()
    ...
    def get_optimization_recommendations() -> List[str]:
        ...
        for bottleneck in bottlenecks:
            ...
            if step == 'content_generation':
                ...
                recommendations.append(
                    "Görsel üretimi yavaş. Batch processing'i etkinleştir veya CDN kullan."
                )
            elif step == 'media_processing':
                recommendations.append(
                    "Medya işleme yavaş. FFmpeg parallelization'ını etkinleştir."
                )
