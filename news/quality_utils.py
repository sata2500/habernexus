...
def calculate_readability(html_content):
    ...
    if sentence_count > 0 and word_count > 0:
        gunning_fog = (
            0.4 * ((word_count / sentence_count) + 100 * (complex_words / word_count))
        )
    ...
    if sentence_count > 0:
        smog = (
            1.0430 * (30 * (complex_words / sentence_count)) ** 0.5 + 3.1291
        )
...
def calculate_seo_metrics(title, html_content, category):
    ...
    primary_keyword_count = len(re.findall(
        r'\b' + re.escape(primary_keyword) + r'\b',
        text_lower
    ))
