...
class ContentQualityMetrics(models.Model):
    ...
    gunning_fog_index = models.FloatField(
        default=0,
        help_text="Gunning Fog İndeksi"
    )

    smog_index = models.FloatField(
        default=0,
        help_text="SMOG İndeksi"
    )

    # İçerik Metrikleri
    word_count = models.IntegerField(
        default=0,
        help_text="Toplam kelime sayısı"
    )

    sentence_count = models.IntegerField(
        default=0,
        help_text="Toplam cümle sayısı"
    )

    paragraph_count = models.IntegerField(
        default=0,
        help_text="Toplam paragraf sayısı"
    )

    avg_sentence_length = models.FloatField(
        default=0,
        help_text="Ortalama cümle uzunluğu (kelime)"
    )

    avg_word_length = models.FloatField(
        default=0,
        help_text="Ortalama kelime uzunluğu (karakter)"
    )
    ...
    primary_keyword = models.CharField(
        max_length=100,
        blank=True,
        help_text="Birincil anahtar kelime"
    )

    primary_keyword_count = models.IntegerField(
        default=0,
        help_text="Birincil anahtar kelime kullanım sayısı"
    )

    secondary_keyword_count = models.IntegerField(
        default=0,
        help_text="İkincil anahtar kelime kullanım sayısı"
    )
    ...
    meta_description_length = models.IntegerField(
        default=0,
        help_text="Meta açıklaması uzunluğu"
    )

    has_internal_links = models.BooleanField(
        default=False,
        help_text="İç bağlantılar var mı?"
    )

    internal_link_count = models.IntegerField(
        default=0,
        help_text="İç bağlantı sayısı"
    )

    # Yapı Metrikleri
    has_headings = models.BooleanField(
        default=False,
        help_text="Başlıklar var mı?"
    )

    heading_count = models.IntegerField(
        default=0,
        help_text="Başlık sayısı"
    )

    h2_count = models.IntegerField(
        default=0,
        help_text="H2 başlık sayısı"
    )

    h3_count = models.IntegerField(
        default=0,
        help_text="H3 başlık sayısı"
    )

    has_lists = models.BooleanField(
        default=False,
        help_text="Listeler var mı?"
    )

    list_count = models.IntegerField(
        default=0,
        help_text="Liste sayısı"
    )

    has_images = models.BooleanField(
        default=False,
        help_text="Görseller var mı?"
    )

    image_count = models.IntegerField(
        default=0,
        help_text="Görsel sayısı"
    )

    has_bold_text = models.BooleanField(
        default=False,
        help_text="Kalın metin var mı?"
    )

    bold_count = models.IntegerField(
        default=0,
        help_text="Kalın metin sayısı"
    )
    ...
    overall_quality_score = models.FloatField(
        default=0,
        validators=[MinValueValidator(0), MaxValueValidator(100)],
        help_text="Genel kalite puanı (0-100)"
    )
...
class HeadlineScore(models.Model):
    ...
    rss_source = models.ForeignKey(
        RssSource,
        on_delete=models.CASCADE,
        related_name='headline_scores',
        help_text="Başlığın kaynağı"
    )

    original_headline = models.CharField(
        max_length=500,
        help_text="Orijinal başlık metni"
    )
    ...
    relevance_score = models.FloatField(
        default=0,
        validators=[MinValueValidator(0), MaxValueValidator(100)],
        help_text="Konu uygunluğu puanı"
    )

    uniqueness_score = models.FloatField(
        default=0,
        validators=[MinValueValidator(0), MaxValueValidator(100)],
        help_text="Orijinallik puanı"
    )

    engagement_score = models.FloatField(
        default=0,
        validators=[MinValueValidator(0), MaxValueValidator(100)],
        help_text="Katılım potansiyeli puanı"
    )

    keyword_relevance = models.FloatField(
        default=0,
        validators=[MinValueValidator(0), MaxValueValidator(100)],
        help_text="Anahtar kelime uygunluğu"
    )

    # Başlık Özellikleri
    word_count = models.IntegerField(
        default=0,
        help_text="Başlıktaki kelime sayısı"
    )

    character_count = models.IntegerField(
        default=0,
        help_text="Başlıktaki karakter sayısı"
    )

    has_numbers = models.BooleanField(
        default=False,
        help_text="Başlıkta sayı var mı?"
    )

    has_power_words = models.BooleanField(
        default=False,
        help_text="Başlıkta güçlü kelimeler var mı?"
    )

    power_words = models.CharField(
        max_length=255,
        blank=True,
        help_text="Bulunan güçlü kelimeler"
    )

    is_question = models.BooleanField(
        default=False,
        help_text="Başlık soru mu?"
    )

    is_listicle = models.BooleanField(
        default=False,
        help_text="Başlık listicle mi? (örn: '5 Yol')"
    )

    # Durum
    is_processed = models.BooleanField(
        default=False,
        help_text="Makaleye dönüştürüldü mü?"
    )
...
class ArticleClassification(models.Model):
    ...
    primary_category = models.CharField(
        max_length=100,
        help_text="Birincil kategori"
    )

    secondary_categories = models.CharField(
        max_length=500,
        blank=True,
        help_text="İkincil kategoriler (virgülle ayrılmış)"
    )
    ...
    research_depth = models.IntegerField(
        ...
        default=1,
        help_text="Gereken araştırma derinliği"
    )

    # AI Model Seçimi
    recommended_ai_model = models.CharField(
        max_length=50,
        default='gemini-2.5-flash',
        help_text="Önerilen AI modeli"
    )

    # Ek Bilgiler
    requires_fact_checking = models.BooleanField(
        default=True,
        help_text="Gerçeklik kontrolü gerekli mi?"
    )

    is_time_sensitive = models.BooleanField(
        default=False,
        help_text="Zamana bağlı haber mi?"
    )

    is_controversial = models.BooleanField(
        default=False,
        help_text="Tartışmalı bir konu mu?"
    )

    target_audience = models.CharField(
        max_length=100,
        blank=True,
        help_text="Hedef kitle"
    )
...
class ResearchSource(models.Model):
    ...
    source_url = models.URLField(
        help_text="Kaynağın URL'si"
    )

    source_title = models.CharField(
        max_length=500,
        help_text="Kaynağın başlığı"
    )
    ...
    relevance_score = models.FloatField(
        default=0,
        validators=[MinValueValidator(0), MaxValueValidator(1)],
        help_text="Makaleye uygunluk puanı (0-1)"
    )

    used_for = models.CharField(
        max_length=255,
        blank=True,
        help_text="Kaynağın hangi amaç için kullanıldığı"
    )

    extracted_text = models.TextField(
        blank=True,
        help_text="Kaynaktan çıkarılan metin"
    )
...
class ContentGenerationLog(models.Model):
    ...
    duration = models.IntegerField(
        default=0,
        help_text="İşlem süresi (milisaniye)"
    )

    input_data = models.JSONField(
        default=dict,
        blank=True,
        help_text="Aşamaya gelen veriler"
    )

    output_data = models.JSONField(
        default=dict,
        blank=True,
        help_text="Aşamadan çıkan veriler"
    )
    ...
    api_calls_count = models.IntegerField(
        default=0,
        help_text="Yapılan API çağrısı sayısı"
    )

    tokens_used = models.IntegerField(
        default=0,
        help_text="Kullanılan token sayısı"
    )
