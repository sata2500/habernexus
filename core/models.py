...
class Setting(models.Model):
    ...
    key = models.CharField(
        max_length=255,
        unique=True,
        help_text="Ayar anahtarı (örn: GOOGLE_API_KEY)"
    )
    value = models.TextField(
        help_text="Ayar değeri"
    )
    description = models.TextField(
        blank=True,
        help_text="Ayarın açıklaması"
    )
    is_secret = models.BooleanField(
        default=False,
        help_text="Gizli bilgi mi? (Yönetim panelinde maskelenecek)"
    )
...
class SystemLog(models.Model):
    ...
    task_name = models.CharField(
        max_length=255,
        help_text="Görevi tetikleyen Celery task'ının adı"
    )
    message = models.TextField(
        help_text="Hata veya olay mesajı"
    )
    traceback = models.TextField(
        blank=True,
        help_text="Hata stack trace'i (varsa)"
    )
    related_id = models.IntegerField(
        null=True,
        blank=True,
        help_text="İlgili nesnenin ID'si (örn: Article ID)"
    )
    created_at = models.DateTimeField(auto_now_add=True)
